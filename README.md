...
2. **本地雙端網頁驗證（強制規範）**：
   - 啟動本地伺服器（如 `python3 -m http.server 8000`）。
   - **手動打開瀏覽器**，並分別針對**桌面版 (Desktop)** 與**行動版 (Mobile RWD)** 進行深度排版、樣式與數據完整性驗證。
   - 應特別檢視表格在行動版下是否具備正確的 `data-label` 欄位標題、文字是否有壓框重疊、按鈕與互動點是否正常。確保雙端無任何視覺或互動瑕疵後，始可進入下一步。

3. **Code Review 與 CHANGELOG 紀錄**：
   - 更新專案根目錄的 `CHANGELOG.md`，記錄此次修改的「紀錄時間」、「問題現狀」、「根本原因 (Root Cause)」、「修正方案」與「驗證結果」（符合 Rule #6 協作協議）。

4. **GitHub Push & Vercel 部署**：
...
5. **線上生產環境即時驗證**：
   - 部署完成後，**必須**使用瀏覽器開啟 Vercel 生產環境網址，在雙端（桌面/行動）視角下再次手動檢驗功能（例如導航選單是否可正常點選、頁面最下方的開發者卡片與品質指標是否渲染正確、航空公司數據是否順利過濾且圓餅圖維度正確）。
